...
// Split LDGSTS issue into A-then-B halves, matching cutedsl's order:
//   [LDSM next kk] [LDGSTS A] [HMMA] [LDGSTS B + commit]
// A and B share the same per-thread (row, chunk) decomposition.
__device__ __forceinline__ void issue_ktile_load_A(
    const half* (&srcA)[8],
    uint32_t sA_addr,
    int tid)
{
    // 8 chunks fully unrolled; each thread has a fixed (row, chunk) per idx.
    #pragma unroll
...
__device__ __forceinline__ void issue_ktile_load_B(
    const half* (&srcB)[8],
    uint32_t sB_addr,
    int tid)
{
           ...
           half* __restrict__ gC)
{
    ...
    for (int s = 0; s < NUM_STAGES - 1; ++s) {
        issue_ktile_load_A(srcA, sA0 + s * STAGE_BYTES, tid);
        issue_ktile_load_B(srcB, sB0 + s * STAGE_BYTES, tid);
    }
...
#define MM_STEP(kk, CUR, NXT)                                                          \
    do {                                                                               \
        ...
        if (kk == 0 && can_issue) {                                                    \
            issue_ktile_load_A(srcA, sA0 + write_stage * STAGE_BYTES, tid);            \
        }                                                                              \
        mma_kk(aBank[CUR], bBank[CUR], C);                                             \
        if (kk == 0 && can_issue) {                                                    \
            issue_ktile_load_B(srcB, sB0 + write_stage * STAGE_BYTES, tid);            \
            k_tile_index++;                                                            \
